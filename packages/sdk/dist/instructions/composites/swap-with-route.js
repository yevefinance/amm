...
async function getSwapFromRoute(ctx, params, opts = fetcher_1.PREFER_CACHE, txBuilder = new common_sdk_1.TransactionBuilder(ctx.connection, ctx.wallet, (0, txn_utils_1.contextOptionsToBuilderOptions)(ctx.opts))) {
    ...
    const uninitializedArrays = await __1.TickArrayUtil.getUninitializedArraysString(requiredTickArrays, ctx.fetcher, opts);
    ...
    for (let i = 0; i < slippageAdjustedQuotes.length; i++) {
        ...
        if (routeFragment.hopQuotes.length === 1) {
            const { quote, yevefi, mintA, mintB, vaultA, vaultB } = routeFragment.hopQuotes[0];
            ...
            const { quote: quoteOne, yevefi: yevefiOne, mintA: mintOneA, mintB: mintOneB, vaultA: vaultOneA, vaultB: vaultOneB, } = routeFragment.hopQuotes[0];
            const { quote: quoteTwo, yevefi: yevefiTwo, mintA: mintTwoA, mintB: mintTwoB, vaultA: vaultTwoA, vaultB: vaultTwoB, } = routeFragment.hopQuotes[1];
