...
class PriceModuleUtils {
    ...
    static async fetchPoolDataFromMints(fetcher, mints, config = _1.defaultGetPricesConfig, opts = fetcher_1.IGNORE_CACHE) {
        const { quoteTokens, tickSpacings, programId, yevefisConfig } = config;
        const poolAddresses = mints.flatMap((mint) => tickSpacings.flatMap((tickSpacing) => {
            return quoteTokens.map((quoteToken) => {
                const [mintA, mintB] = public_1.PoolUtil.orderMints(mint, quoteToken);
                return public_1.PDAUtil.getYevefi(programId, yevefisConfig, common_sdk_1.AddressUtil.toPubKey(mintA), common_sdk_1.AddressUtil.toPubKey(mintB), tickSpacing).publicKey.toBase58();
            });
        }));
