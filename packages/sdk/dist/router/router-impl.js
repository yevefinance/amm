...
exports.YevefiRouterImpl = void 0;
...
class YevefiRouterImpl {
    ...
    async findAllRoutes(trade, opts, fetchOpts) {
        ...
        if (paths.length === 0) {
            return Promise.reject(new errors_1.YevefisError(`Could not find route for ${tokenIn} -> ${tokenOut}`, errors_1.RouteQueryErrorCode.RouteDoesNotExist));
        }
        if (tradeAmount.isZero()) {
            return Promise.reject(new errors_1.YevefisError("findBestRoutes error - input amount is zero.", errors_1.RouteQueryErrorCode.ZeroInputAmount));
        }
        ...
        try {
            ...
            if (bestRoutes.length === 0) {
                ...
                if (failures.has(errors_1.SwapErrorCode.TickArraySequenceInvalid)) {
                    return Promise.reject(new errors_1.YevefisError("All swap quote generation failed on amount too high.", errors_1.RouteQueryErrorCode.TradeAmountTooHigh));
                }
            }
            return bestRoutes;
        }
        catch (e) {
            return Promise.reject(new errors_1.YevefisError("Stack error received on quote generation.", errors_1.RouteQueryErrorCode.General, e.stack));
        }
    }
...
exports.YevefiRouterImpl = YevefiRouterImpl;
