...
class SwapUtils {
     ...
     */
    static getTickArrayPublicKeys(tickCurrentIndex, tickSpacing, aToB, programId, yevefiAddress) {
        ...
        for (let i = 0; i < public_1.MAX_SWAP_TICK_ARRAYS; i++) {
            ...
            const pda = pda_utils_1.PDAUtil.getTickArray(programId, yevefiAddress, startIndex);
     ...
     * @returns An array of PublicKey[] for the tickArray accounts that this swap may traverse across.
     */
    static async getTickArrays(tickCurrentIndex, tickSpacing, aToB, programId, yevefiAddress, fetcher, opts) {
        const data = await SwapUtils.getBatchTickArrays(programId, fetcher, [{ tickCurrentIndex, tickSpacing, aToB, yevefiAddress }], opts);
        return data[0];
    }
    /**
    ...
    static async getBatchTickArrays(programId, fetcher, tickArrayRequests, opts) {
        ...
        for (let i = 0; i < tickArrayRequests.length; i++) {
            const { tickCurrentIndex, tickSpacing, aToB, yevefiAddress } = tickArrayRequests[i];
            const requestAddresses = SwapUtils.getTickArrayPublicKeys(tickCurrentIndex, tickSpacing, aToB, programId, yevefiAddress);
     ...
     */
    static getSwapParamsFromQuote(quote, ctx, yevefi, inputTokenAssociatedAddress, outputTokenAssociatedAddress, wallet) {
        const data = yevefi.getData();
        return SwapUtils.getSwapParamsFromQuoteKeys(quote, ctx, yevefi.getAddress(), data.tokenVaultA, data.tokenVaultB, inputTokenAssociatedAddress, outputTokenAssociatedAddress, wallet);
    }
    static getSwapParamsFromQuoteKeys(quote, ctx, yevefi, tokenVaultA, tokenVaultB, inputTokenAssociatedAddress, outputTokenAssociatedAddress, wallet) {
        ...
        const oraclePda = pda_utils_1.PDAUtil.getOracle(ctx.program.programId, yevefi);
        const params = {
            yevefi,
