...
class TickArrayUtil {
     ...
     */
    static getTickArrayPDAs(tick, tickSpacing, numOfTickArrays, programId, yevefiAddress, aToB) {
        ...
        return arrayIndexList.map((value) => {
            ...
            return pda_utils_1.PDAUtil.getTickArray(programId, yevefiAddress, startTick);
        });
    }
    /**
    ...
    static async getUninitializedArraysPDAs(ticks, programId, yevefiAddress, tickSpacing, fetcher, opts) {
        const startTicks = ticks.map((tick) => TickUtil.getStartTickIndex(tick, tickSpacing));
        const removeDupeTicks = [...new Set(startTicks)];
        const tickArrayPDAs = removeDupeTicks.map((tick) => pda_utils_1.PDAUtil.getTickArray(programId, yevefiAddress, tick));
