...
class TickArraySequence {
    ...
    getTick(index) {
        ...
        if (!tickArray) {
            throw new errors_1.YevefisError(`TickArray at index ${localArrayIndex} is not initialized.`, errors_1.SwapErrorCode.TickArrayIndexNotInitialized);
        }
        if (!this.checkIfIndexIsInTickArrayRange(tickArray.startTickIndex, index)) {
            throw new errors_1.YevefisError(`TickArray at index ${localArrayIndex} is unexpected for this sequence.`, errors_1.SwapErrorCode.TickArraySequenceInvalid);
        }
    ...
    findNextInitializedTickIndex(currIndex) {
        ...
        if (!this.isArrayIndexInBounds(currTaIndex, this.aToB)) {
            throw new errors_1.YevefisError(`Swap input value traversed too many arrays. Out of bounds at attempt to traverse tick index - ${currTaIndex.toTickIndex()}.`, errors_1.SwapErrorCode.TickArraySequenceInvalid);
        }
