 ...
 */
function increaseLiquidityQuoteByInputTokenUsingPriceSlippage(inputTokenMint, inputTokenAmount, tickLower, tickUpper, slippageTolerance, yevefi) {
...
function getLiquidityFromInputToken(params) {
    ...
    if (positionStatus === position_util_1.PositionStatus.BelowRange) {
        return isTokenA
            ? (0, position_util_1.getLiquidityFromTokenA)(inputTokenAmount, sqrtPriceLowerX64, sqrtPriceUpperX64, false)
            : common_sdk_1.ZERO;
    }
    if (positionStatus === position_util_1.PositionStatus.AboveRange) {
        return isTokenA
            ? common_sdk_1.ZERO
            : (0, position_util_1.getLiquidityFromTokenB)(inputTokenAmount, sqrtPriceLowerX64, sqrtPriceUpperX64, false);
    }
    return isTokenA
 ...
 */
function increaseLiquidityQuoteByInputToken(inputTokenMint, inputTokenAmount, tickLower, tickUpper, slippageTolerance, yevefi) {
    const data = yevefi.getData();
    const tokenAInfo = yevefi.getTokenAInfo();
    const tokenBInfo = yevefi.getTokenBInfo();
    const inputMint = common_sdk_1.AddressUtil.toPubKey(inputTokenMint);
