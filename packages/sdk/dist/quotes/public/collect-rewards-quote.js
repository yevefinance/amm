...
function collectRewardsQuote(param) {
    ...
    const { tickCurrentIndex, rewardInfos: yevefiRewardsInfos, rewardLastUpdatedTimestamp, } = yevefi;
    ...
    for (let i = 0; i < public_1.NUM_REWARDS; i++) {
        ...
        (0, tiny_invariant_1.default)(!!rewardInfo, "yevefiRewardsInfos cannot be undefined");
        ...
        if (!yevefi.liquidity.isZero()) {
            const rewardGrowthDelta = bit_math_1.BitMath.mulDiv(timestampDelta, rewardInfo.emissionsPerSecondX64, yevefi.liquidity, 128);
