...
exports.YevefiClientImpl = void 0;
...
const yevefi_impl_1 = require("./yevefi-impl");
class YevefiClientImpl {
    constructor(ctx) {
        this.ctx = ctx;
    }
    getContext() {
    ...
    getRouter(poolAddresses) {
        return public_1.YevefiRouterBuilder.buildWithPools(this.ctx, poolAddresses);
    }
    async getPool(poolAddress, opts = fetcher_1.PREFER_CACHE) {
        ...
        return new yevefi_impl_1.YevefiImpl(this.ctx, common_sdk_1.AddressUtil.toPubKey(poolAddress), tokenInfos[0], tokenInfos[1], vaultInfos[0], vaultInfos[1], rewardInfos, account);
    }
    async getPools(poolAddresses, opts = fetcher_1.PREFER_CACHE) {
        ...
        if (accounts.length !== poolAddresses.length) {
            throw new Error(`Unable to fetch all Yevefis at addresses ${poolAddresses}`);
        }
        ...
        for (let i = 0; i < accounts.length; i++) {
            ...
            yevefis.push(new yevefi_impl_1.YevefiImpl(this.ctx, common_sdk_1.AddressUtil.toPubKey(poolAddress), tokenInfos[0], tokenInfos[1], vaultInfos[0], vaultInfos[1], rewardInfos, account));
        }
        return yevefis;
    }
    async getPosition(positionAddress, opts = fetcher_1.PREFER_CACHE) {
        ...
        if (!whirlAccount) {
            throw new Error(`Unable to fetch Yevefi for Position at address at ${positionAddress}`);
        }
    ...
    async getPositions(positionAddresses, opts = fetcher_1.PREFER_CACHE) {
        ...
        await Promise.all(positions.map(async (pos) => {
            if (pos) {
                const pool = await this.ctx.fetcher.getPool(pos.yevefi, fetcher_1.PREFER_CACHE);
                if (pool) {
                    const lowerTickArrayPda = public_2.PDAUtil.getTickArrayFromTickIndex(pos.tickLowerIndex, pool.tickSpacing, pos.yevefi, this.ctx.program.programId).publicKey;
                    const upperTickArrayPda = public_2.PDAUtil.getTickArrayFromTickIndex(pos.tickUpperIndex, pool.tickSpacing, pos.yevefi, this.ctx.program.programId).publicKey;
    ...
    async createPool(yevefisConfig, tokenMintA, tokenMintB, tickSpacing, initialTick, funder, opts = fetcher_1.PREFER_CACHE) {
        ...
        yevefisConfig = common_sdk_1.AddressUtil.toPubKey(yevefisConfig);
        const feeTierKey = public_2.PDAUtil.getFeeTier(this.ctx.program.programId, yevefisConfig, tickSpacing).publicKey;
        ...
        const yevefiPda = public_2.PDAUtil.getYevefi(this.ctx.program.programId, yevefisConfig, new web3_js_1.PublicKey(tokenMintA), new web3_js_1.PublicKey(tokenMintB), tickSpacing);
        ...
        const initPoolIx = ix_1.YevefiIx.initializePoolIx(this.ctx.program, {
            initSqrtPrice,
            yevefisConfig,
            yevefiPda,
        ...
        const initialTickArrayPda = public_2.PDAUtil.getTickArray(this.ctx.program.programId, yevefiPda.publicKey, initialTickArrayStartTick);
...
exports.YevefiClientImpl = YevefiClientImpl;
