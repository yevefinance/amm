...
class YevefiImpl {
    ...
    async openPositionWithMetadata(tickLower, tickUpper, liquidityInput, sourceWallet, funder, positionMint) {
        ...
        return this.getOpenPositionWithOptMetadataTx(tickLower, tickUpper, liquidityInput, sourceWallet
            ...
            : this.ctx.wallet.publicKey, funder ? common_sdk_1.AddressUtil.toPubKey(funder) : this.ctx.wallet.publicKey, true, positionMint);
    }
    ...
    async getOpenPositionWithOptMetadataTx(tickLower, tickUpper, liquidityInput, wallet, funder, withMetadata = false, positionMint) {
        ...
        const yevefi = await this.ctx.fetcher.getPool(this.address, fetcher_1.PREFER_CACHE);
        if (!yevefi) {
            throw new Error(`Yevefi not found: ${(0, anchor_1.translateAddress)(this.address).toBase58()}`);
        }
        (0, tiny_invariant_1.default)(public_2.TickUtil.isTickInitializable(tickLower, yevefi.tickSpacing), `lower tick ${tickLower} is not an initializable tick for tick-spacing ${yevefi.tickSpacing}`);
        (0, tiny_invariant_1.default)(public_2.TickUtil.isTickInitializable(tickUpper, yevefi.tickSpacing), `upper tick ${tickUpper} is not an initializable tick for tick-spacing ${yevefi.tickSpacing}`);
    ...
    async getClosePositionIx(positionAddress, slippageTolerance, destinationWallet, positionWallet, payerKey) {
        ...
        (0, tiny_invariant_1.default)(positionData.yevefi.equals(this.address), `Position ${positionAddress.toBase58()} is not a position for Yevefi ${this.address.toBase58()}`);
        ...
        const txBuilder = new common_sdk_1.TransactionBuilder(this.ctx.provider.connection, this.ctx.provider.wallet, this.ctx.txBuilderOpts);
        const tickArrayLower = public_2.PDAUtil.getTickArrayFromTickIndex(positionData.tickLowerIndex, yevefi.tickSpacing, positionData.yevefi, this.ctx.program.programId).publicKey;
        const tickArrayUpper = public_2.PDAUtil.getTickArrayFromTickIndex(positionData.tickUpperIndex, yevefi.tickSpacing, positionData.yevefi, this.ctx.program.programId).publicKey;
        const [tickArrayLowerData, tickArrayUpperData] = await (0, position_builder_util_1.getTickArrayDataForPosition)(this.ctx, positionData, yevefi, fetcher_1.IGNORE_CACHE);
        (0, tiny_invariant_1.default)(!!tickArrayLowerData, `Tick array ${tickArrayLower} expected to be initialized for yevefi ${this.address}`);
        (0, tiny_invariant_1.default)(!!tickArrayUpperData, `Tick array ${tickArrayUpper} expected to be initialized for yevefi ${this.address}`);
        const position = new position_impl_1.PositionImpl(this.ctx, positionAddress, positionData, yevefi, tickArrayLowerData, tickArrayUpperData);
        ...
        let mintType = yevefi_ata_utils_1.TokenMintTypes.ALL;
        if ((shouldDecreaseLiquidity || shouldCollectFees) &&
            !shouldCollectRewards) {
            mintType = yevefi_ata_utils_1.TokenMintTypes.POOL_ONLY;
        }
        else if (!(shouldDecreaseLiquidity || shouldCollectFees) &&
            shouldCollectRewards) {
            mintType = yevefi_ata_utils_1.TokenMintTypes.REWARD_ONLY;
        }
        const affiliatedMints = (0, yevefi_ata_utils_1.getTokenMintsFromYevefis)([yevefi], mintType);
        const { ataTokenAddresses: walletTokenAccountsByMint, resolveAtaIxs } = await (0, yevefi_ata_utils_1.resolveAtaForMints)(this.ctx, {
        ...
        if (affiliatedMints.hasNativeMint) {
            const { address: wSOLAta, ...resolveWSolIx } = common_sdk_1.TokenUtil.createWrappedNativeAccountInstruction(destinationWallet, common_sdk_1.ZERO, accountExemption, payerKey, destinationWallet, this.ctx.accountResolverOpts.createWrappedSolAccountMethod);
        ...
        if (shouldDecreaseLiquidity) {
            ...
            const tokenOwnerAccountA = walletTokenAccountsByMint[yevefi.tokenMintA.toBase58()];
            const tokenOwnerAccountB = walletTokenAccountsByMint[yevefi.tokenMintB.toBase58()];
