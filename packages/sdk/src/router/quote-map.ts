...
import { type SwapQuoteParam, swapQuoteWithParams } from "../quotes/public";
import { type Path, PoolUtil } from "../utils/public";
import {
	type SwapQuoteRequest,
	batchBuildSwapQuoteParams,
...
function populateQuoteMap(
	...
	for (const {
		...
		try {
			...
			const { yevefiData, tokenAmount, aToB, amountSpecifiedIsInput } =
				swapParam;
			const [mintA, mintB, vaultA, vaultB] = [
				yevefiData.tokenMintA.toBase58(),
				yevefiData.tokenMintB.toBase58(),
				yevefiData.tokenVaultA.toBase58(),
				yevefiData.tokenVaultB.toBase58(),
			];
			...
			path.calculatedEdgeQuotes[edgeIndex] = {
				...
				snapshot: {
					aToB: swapParam.aToB,
					sqrtPrice: yevefiData.sqrtPrice,
					feeRate: PoolUtil.getFeeRate(yevefiData.feeRate),
				},
			};
		} catch (e: any) {
...
function getSplitPercentageAmts(inputAmount: BN, minPercent = 5) {
	const percents = [];
	const amounts = [];
