...
import type { YevefiData, YevefiRewardInfoData } from "../../types/public";
...
export class PoolUtil {
	...
	public static isRewardInitialized(rewardInfo: YevefiRewardInfoData): boolean {
		return (
	...
	/**
	 * Return the corresponding token type (TokenA/B) for this mint key for a Yevefi.
	 *
	...
	public static getTokenType(
		...
		if (pool.tokenMintB.equals(mint)) {
	...
	public static getFeeRate(feeRate: number): Percentage {
		/**
		 * Smart Contract comment: https://github.com/orca-so/yevefi/blob/main/programs/yevefi/src/state/yevefi.rs#L9-L11
	...
	public static getProtocolFeeRate(protocolFeeRate: number): Percentage {
		/**
		 * Smart Contract comment: https://github.com/orca-so/yevefi/blob/main/programs/yevefi/src/state/yevefi.rs#L13-L14
	...
	public static orderMints(mintX: Address, mintY: Address): [Address, Address] {
		return PoolUtil.compareMints(mintX, mintY) < 0
			? [mintX, mintY]
			: [mintY, mintX];
	}
	 ...
	 *
	 * @category Yevefi Utils
	 * @param currTick - Yevefi's current tick index (aka price)
