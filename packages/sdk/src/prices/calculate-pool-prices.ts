...
import {
	type DecimalsMap,
	type GetPricesConfig,
	type GetPricesThresholdConfig,
	type PoolMap,
	type PriceMap,
	type TickArrayMap,
...
type PoolObject = { pool: YevefiData; address: PublicKey };
...
function getTickArrays(
	...
	return tickArrayPublicKeys.map((tickArrayPublicKey) => {
		return {
			address: tickArrayPublicKey,
			data: tickArrayMap[tickArrayPublicKey.toBase58()],
		};
	});
}

function getPrice(pool: YevefiData, decimalsMap: DecimalsMap) {
