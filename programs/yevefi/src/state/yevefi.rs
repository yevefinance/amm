...
use super::YevefisConfig;

#[account]
#[derive(Default)]
pub struct Yevefi {
    pub yevefis_config: Pubkey, // 32
    pub yevefi_bump: [u8; 1],   // 1
    ...
    pub reward_infos: [YevefiRewardInfo; NUM_REWARDS], // 384
}

// Number of rewards supported by Yevefis
...
impl Yevefi {
    ...
    /// Update all reward values for the Yevefi.
    ///
    /// # Parameters
    /// - `reward_infos` - An array of all updated yevefi rewards
    ...
    /// Update the reward authority at the specified Yevefi reward index.
...
/// Stores the state relevant for tracking liquidity mining rewards at the `Yevefi` level.
/// These values are used in conjunction with `PositionRewardInfo`, `Tick.reward_growths_outside`,
/// and `Yevefi.reward_last_updated_timestamp` to determine how many rewards are earned by open
/// positions.
#[derive(Copy, Clone, AnchorSerialize, AnchorDeserialize, Default, Debug, PartialEq)]
pub struct YevefiRewardInfo {
    /// Reward token mint.
    pub mint: Pubkey,
...
impl YevefiRewardInfo {
    /// Creates a new `YevefiRewardInfo` with the authority set
...
#[cfg(test)]
pub mod yevefi_builder {
    use super::{Yevefi, YevefiRewardInfo, NUM_REWARDS};

    #[derive(Default)]
    pub struct YevefiBuilder {
        liquidity: u128,
        tick_spacing: u16,
    ...
    impl YevefiBuilder {
        pub fn new() -> Self {
            Self {
                reward_infos: [YevefiRewardInfo::default(); NUM_REWARDS],
                ..Default::default()
            }
        }
        ...
        pub fn reward_info(mut self, index: usize, reward_info: YevefiRewardInfo) -> Self {
            self.reward_infos[index] = reward_info;
            self
        }

        pub fn reward_infos(mut self, reward_infos: [YevefiRewardInfo; NUM_REWARDS]) -> Self {
            self.reward_infos = reward_infos;
            self
        }
