...
impl Position {
    ...
    pub fn open_position(
        ...
        if yevefi.tick_spacing >= FULL_RANGE_ONLY_TICK_SPACING_THRESHOLD {
            let (full_range_lower_index, full_range_upper_index) =
                Tick::full_range_indexes(yevefi.tick_spacing);
...
mod data_layout_tests {
    ...
    fn test_position_data_layout() {
        let position_yevefi = Pubkey::new_unique();
        ...
        offset += 8;
        position_data[offset..offset + 32].copy_from_slice(&position_yevefi.to_bytes());
        offset += 32;
        ...
        assert_eq!(position_yevefi, deserialized.yevefi);
