...
impl Tick {
    ...
    /// Check that the tick index is a valid start tick for a tick array in this yevefi
    /// A valid start-tick-index is a multiple of tick_spacing & number of ticks in a tick-array.
    ///
    /// # Parameters
    /// - `tick_index` - A i32 integer representing the tick index
    /// - `tick_spacing` - A u8 integer of the tick spacing for this yevefi
    ///
    /// # Returns
    /// - `true`: The tick index is a valid start-tick-index for this yevefi
    /// - `false`: The tick index is not a valid start-tick-index for this yevefi
...
impl TickArray {
    ...
    ///
    /// # Parameters
    /// - `yevefi` - the tick index the desired Tick object is stored in
    /// - `start_tick_index` - A u8 integer of the tick spacing for this yevefi
    ///
    /// # Errors
    /// - `InvalidStartTick`: - The provided start-tick-index is not an initializable tick index in this Yevefi w/ this tick-spacing.
    pub fn initialize(&mut self, yevefi: &Account<Yevefi>, start_tick_index: i32) -> Result<()> {
        if !Tick::check_is_valid_start_tick(start_tick_index, yevefi.tick_spacing) {
...
impl TickArrayType for TickArray {
    ...
    /// - `TickNotFound`: - The provided tick-index is not an initializable tick index in this Yevefi w/ this tick-spacing.
    fn get_tick(&self, tick_index: i32, tick_spacing: u16) -> Result<&Tick> {
...
mod data_layout_tests {
    ...
    fn test_tick_array_data_layout() {
        ...
        let tick_array_yevefi = Pubkey::new_unique();
        ...
        tick_array_data[offset..offset + 32].copy_from_slice(&tick_array_yevefi.to_bytes());
        offset += 32;
        ...
        let read_yevefi = tick_array.yevefi;
        assert_eq!(read_yevefi, tick_array_yevefi);
    }
}
