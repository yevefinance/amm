...
use crate::constants::{
    TK_2022_METADATA_NAME_PREFIX, TK_2022_METADATA_SYMBOL, TK_2022_METADATA_URI_BASE,
};
use crate::state::*;
...
pub fn initialize_token_metadata_extension<'info>(
    ...
    // update authority: TK_NFT_UPDATE_AUTH
    invoke_signed(
...
pub fn build_position_token_metadata<'info>(
    ...
    // TK_2022_METADATA_NAME_PREFIX + " xxxx...yyyy"
    // xxxx and yyyy are the first and last 4 chars of mint address
    let mint_address = position_mint.key().to_string();
    let name = format!(
        "{} {}...{}",
        TK_2022_METADATA_NAME_PREFIX,
        &mint_address[0..4],
        &mint_address[mint_address.len() - 4..],
    );

    // TK_2022_METADATA_URI_BASE + "/" + pool address + "/" + position address
    // Must be less than 128 bytes
    let uri = format!(
        "{}/{}/{}",
        TK_2022_METADATA_URI_BASE,
        yevefi.key(),
        position.key(),
    );

    (name, TK_2022_METADATA_SYMBOL.to_string(), uri)
}
